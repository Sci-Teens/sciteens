...
describe('fetchDeadlineUnknownOpportunities', () => {
  it('queries exactly the unclear bucket', async () => {
    getDocs.mockResolvedValueOnce(snapshotOf([]))
    await fetchDeadlineUnknownOpportunities({})
    const built = getDocs.mock.calls.at(-1)[0]
    expect(built.base).toEqual({
  ...
  it('returns normalized docs keyed by slug', async () => {
    ...
    const results = await fetchDeadlineUnknownOpportunities(
      {}
    )
    expect(results).toEqual([
      {
        slug: 'mystery-program',
        name: 'Mystery Program',
        deadlineStatus: 'unclear',
        applicationDeadline: null,
        applicationOpensDate: null,
