...
export function buildOpportunitySearchFilter({
  ...
  const clauses = [statusFilter(status, now)]
  if (field)
  ...
  if (location) {
    clauses.push(quotedFilter('location_facets', location))
  }
  if (programType) {
...
export function buildOpportunitySearchQueries(
  ...
  return [
    ...
    buildFacetQuery(options, 'location', 'location_facets'),
