...
// Covers the former pages/api/toxicity.js Perspective proxy, adapted to
// the client-side Xenova/toxic-bert classifier (Transformers.js, running
// in lib/toxicityWorker.js) that replaced it — see
// https://web.dev/articles/ai-detect-toxicity-build.
//
// Several original bullets no longer apply because there is no longer
// an HTTP route at all: method/origin checks, per-IP rate limiting, and
// "missing GM_API_KEY -> 500" all belonged to the deleted server proxy.
// There is nothing left to fail open a request or leak a secret with —
// classification runs entirely in the user's own browser, off a public
// model name, with no server secret in the loop. What carries over 1:1
// is the input validation ("missing/empty text -> 400", "text over
// MAX_TEXT_LEN -> 400") and — more importantly, since it's the actual
// detection logic that used to live behind the proxy — the
