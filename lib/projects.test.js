...
describe('normalizeProject links allowlist', () => {
  it('keeps allowlisted links (github, youtube)', () => {
    const links = [
      'https://github.com/sciteens/sciteens',
      'https://youtu.be/abc123',
    ]
    expect(normalizeProject({ links }).links).toEqual(links)
  })

  it('drops links to hosts outside the allowlist', () => {
    expect(
      normalizeProject({
        links: [
    ...
    ).toEqual(['https://github.com/sciteens'])
  })

  it('defaults to an empty array when links is missing or malformed', () => {
    expect(normalizeProject({}).links).toEqual([])
    expect(
      normalizeProject({ links: 'not-an-array' }).links
    ).toEqual([])
  })
})
