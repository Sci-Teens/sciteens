...
function loadCourses() {
  ...
  if (courseCache && courseStamp === stamp)
    return courseCache
  // Dimensions belong to the files the corpus references, so they expire with
  // it; otherwise a replaced image keeps its old size in `next dev`.
  measured = new Map()
...
// `text` is the whole course body plus its lesson titles, lowercased once so
// the listing does not redo it per keystroke. Prismic's fulltext predicate
// searched the document, and three courses of prose is ~6 kB, so this ships in
// props instead of the separate corpus the 135 articles need.
